...
| `replace()`        | Replaces the value of specified key from the dictionary with a new value.              | `hashMapExample.replace("a",3);`                                | Changes the key-value pair `"a"=1` to "a"=3 from `hashMapExample`.    |
